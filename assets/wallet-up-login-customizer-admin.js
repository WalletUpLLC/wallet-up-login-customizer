...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('#export-settings').on('click', function(e) {
            ...
            // Collect data from ALL tabs (including hidden ones)
            $('.settings-panel').find('input, select, textarea').each(function() {
                var $field = $(this);
                var name = $field.attr('name');
                if (!name) return;
                var matches = name.match(/^(wallet_up_login_customizer(?:_security)?_options)\[([^\]]+)\](?:\[\])?$/);
                if (!matches) return;
                var optionGroup = matches[1];
                var fieldKey = matches[2];
                var value;
                // Map to correct export structure
                var targetSettings;
                if (optionGroup === 'wallet_up_login_customizer_options') {
                    targetSettings = exportData.settings.login_options;
                } else if (optionGroup === 'wallet_up_login_customizer_security_options') {
                    targetSettings = exportData.settings.security_options;
                } else {
                    console.warn('Unknown option group:', optionGroup);
                    return;
                }
