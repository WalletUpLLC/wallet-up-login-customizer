...
class WalletUpLoginCustomizer {
    ...
    public function enqueue_scripts() {
        ...
        foreach ($this->settings['loading_messages'] as $message) {
            ...
            if ($message === 'Verifying your credentials...') {
                ...
                // Sanitize any custom messages
                $translated_messages[] = sanitize_text_field($message);
            }
        }
    ...
    public function add_dynamic_styles() {
        ...
        // Generate CSP nonce for inline styles (must match CSP header nonce)
        $csp_nonce = wp_create_nonce('wallet-up-csp');
        $nonce_attr = ' nonce="' . esc_attr($csp_nonce) . '"';

        // Critical inline CSS - no FOUC prevention for now
        $critical_css = '';
        ...
        echo '<style type="text/css"' . $nonce_attr . '>' . $css . '</style>';
    }
    ...
    public function login_footer() {
        ...
        // Add CSP nonce to inline script
        $script_nonce = wp_create_nonce('wallet-up-csp');
        echo '<script nonce="' . esc_attr($script_nonce) . '">
