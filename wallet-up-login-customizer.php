<?php
/**
 * Plugin Name: Wallet Up Login Customizer
 * Plugin URI: https://walletup.app/go/customizer
...
class WalletUpLogin {
    ...
    private function __construct() {
        try {
            ...
            // Initialize the plugin - use early priority to ensure force login works
            add_action('plugins_loaded', array($this, 'init'), 1);
            
            // Initialize enterprise security early for force login
            add_action('init', array($this, 'early_security_init'), 0);
    ...
    /**
     * Early initialization for security features that need to run before template redirect
     */
    public function early_security_init() {
        // Only initialize enterprise security if not already done
        static $security_initialized = false;
...
private function init_admin() {
    ...
    // Migrate/ensure email settings exist
    add_action('admin_init', array($this, 'migrate_security_options'));
    ...
    add_action('wp_ajax_wallet_up_export_settings', array($this, 'ajax_export_settings'));
    add_action('wp_ajax_wallet_up_import_settings', array($this, 'ajax_import_settings'));
    add_action('wp_ajax_wallet_up_reset_settings', array($this, 'ajax_reset_settings'));
    
    // AJAX handler for test email
    add_action('wp_ajax_wallet_up_test_security_email', array($this, 'ajax_test_security_email'));
}
    ...
    foreach ($file_mappings as $file) {
        ...
        if (!file_exists($file['source'])) {
            ...
            if (file_exists($file['dest'])) {
                ...
                if (!copy($file['dest'], $file['source'])) {
                    error_log('WalletUp Login: Failed to restore source file: ' . sanitize_text_field(basename($file['source'])));
                }
                $updates_log[] = array(
        ...
        if ($needs_copy) {
            ...
            if ($copy_result) {
                ...
                $errors[] = sprintf(esc_html__('Failed to copy %s', 'wallet-up-login-customizer'), basename($file['source']));
                if ($file['critical']) {
                    $success = false;
                }
            }
...
private function safe_file_copy($source, $destination) {
    ...
    if (file_exists($destination) && !is_writable($destination)) {
        if (!unlink($destination)) {
            error_log('WalletUp Login: Failed to remove existing file: ' . sanitize_text_field(basename($destination)));
            return false;
        }
    }
    ...
    if (!$result) {
        error_log('WalletUp Login: Failed to copy file from ' . sanitize_text_field(basename($source)) . ' to ' . sanitize_text_field(basename($destination)));
        return false;
    }
    
    // Set proper permissions with error handling
    if ($result && !chmod($destination, 0644)) {
        error_log('WalletUp Login: Failed to set permissions on ' . sanitize_text_field(basename($destination)));
    }
...
private function get_asset_url($path) {
    ...
    /**
     * Migrate security options to ensure all fields exist
     */
    public function migrate_security_options() {
        $options = get_option('wallet_up_login_customizer_security_options', array());
        $updated = false;
        ...
        foreach ($email_defaults as $key => $default_value) {
    ...
    public function sanitize_options($input) {
       ...
       // Sanitize checkboxes
        ...
        // Sanitize security settings if present
        if (isset($input['force_login_enabled'])) {
            $sanitized['force_login_enabled'] = (bool) $input['force_login_enabled'];
        }
        if (isset($input['hide_wp_login'])) {
            $sanitized['hide_wp_login'] = (bool) $input['hide_wp_login'];
        }
        if (isset($input['custom_login_slug'])) {
            $slug = sanitize_title($input['custom_login_slug']);
            // Ensure it's not empty and doesn't conflict with WordPress
        ...
        if (isset($input['max_login_attempts'])) {
            $attempts = absint($input['max_login_attempts']);
            $sanitized['max_login_attempts'] = max(3, min(10, $attempts)); // Between 3 and 10
        }
        if (isset($input['lockout_duration'])) {
            $sanitized['lockout_duration'] = absint($input['lockout_duration']);
        }
        // Handle checkbox - if not set, it means unchecked (false)
        $sanitized['email_notifications_enabled'] = isset($input['email_notifications_enabled']) ? true : false;
        // Always set threshold with default
        if (isset($input['email_notification_threshold']) && !empty($input['email_notification_threshold'])) {
            $threshold = absint($input['email_notification_threshold']);
            $sanitized['email_notification_threshold'] = max(1, min(10, $threshold)); // Between 1 and 10
        } else {
            $sanitized['email_notification_threshold'] = 3; // Default value
        }
        // Always set email_recipient, use admin email as fallback
        if (isset($input['email_recipient']) && !empty($input['email_recipient'])) {
            $email = sanitize_email($input['email_recipient']);
            if (is_email($email)) {
                $sanitized['email_recipient'] = $email;
            } else {
                $sanitized['email_recipient'] = get_option('admin_email');
            }
        } else {
            $sanitized['email_recipient'] = get_option('admin_email');
        }
        // Handle checkbox - if not set, it means unchecked (false)
        $sanitized['email_digest_enabled'] = isset($input['email_digest_enabled']) ? true : false;
        
        // Handle checkbox - if not set, it means unchecked (false)
        $sanitized['security_headers'] = isset($input['security_headers']) ? true : false;
    ...
    public function sanitize_security_options($input) {
        // Get existing options to preserve any fields not in the form
        $existing = get_option('wallet_up_login_customizer_security_options', array());
        ...
        $sanitized['exempt_roles'] = isset($input['exempt_roles']) ? $input['exempt_roles'] : ['administrator'];
        
        // Handle EMAIL NOTIFICATION FIELDS
        $sanitized['email_notifications_enabled'] = isset($input['email_notifications_enabled']) && $input['email_notifications_enabled'] ? true : false;
        
        $sanitized['email_notification_threshold'] = isset($input['email_notification_threshold']) ? 
            max(1, min(10, absint($input['email_notification_threshold']))) : 3;
            
        $sanitized['email_recipient'] = isset($input['email_recipient']) && is_email($input['email_recipient']) ? 
            sanitize_email($input['email_recipient']) : get_option('admin_email');
            
        $sanitized['email_digest_enabled'] = isset($input['email_digest_enabled']) && $input['email_digest_enabled'] ? true : false;
        
        $sanitized['security_headers'] = isset($input['security_headers']) && $input['security_headers'] ? true : false;
        
        // Handle whitelist IPs - convert textarea input to array
        if (isset($input['whitelist_ips'])) {
            $ips_raw = $input['whitelist_ips'];
            if (is_string($ips_raw)) {
                // Split by newlines and clean up
                $ips = array_filter(array_map('trim', explode("\n", $ips_raw)));
                // Validate each IP
                $sanitized['whitelist_ips'] = array();
                foreach ($ips as $ip) {
                    // Basic IP validation (supports both IPv4 and IPv6)
                    if (filter_var($ip, FILTER_VALIDATE_IP)) {
                        $sanitized['whitelist_ips'][] = $ip;
                    }
                }
            } elseif (is_array($ips_raw)) {
                // Already an array, just validate
                $sanitized['whitelist_ips'] = array_filter($ips_raw, function($ip) {
        ...
        if (in_array($name, $wallet_up_required)) {
            ...
            if (!$wallet_up_available) {
                ...
                $message = ($name === 'redirect_to_wallet_up') 
                    ? esc_html__('Wallet Up Pro plugin is not active. Please install and activate it to redirect users to Wallet Up after login.', 'wallet-up-login-customizer')
                    : esc_html__('Wallet Up Pro plugin is not active. Please install and activate it to replace the WordPress dashboard.', 'wallet-up-login-customizer');
    ...
    public function render_settings_page() {
        ...
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wallet-up-login-customizer'));
        }
...
.settings-save-wrapper .button-primary {
    background: linear-gradient(135deg, #674FBF 0%, #7B68D4 100%);
    border: none;
    padding: 10px 30px;
    font-size: 14px;
    font-weight: 500;
    border-radius: 6px;
    box-shadow: 0 2px 4px rgba(103, 79, 191, 0.2);
}
.settings-save-wrapper .button-primary:hover {
    background: linear-gradient(135deg, #5543a3 0%, #674FBF 100%);
    box-shadow: 0 4px 8px rgba(103, 79, 191, 0.3);
}
.settings-actions {
    margin-top: 30px;
    padding-top: 30px;
...
<div class="wallet-up-premium-header-wrapper">
    ...
    <div class="wallet-up-admin-container">
        <div class="wallet-up-admin-content">
            ...
            <div id="wallet-up-settings-form">
                <div class="wallet-up-tab-nav">
                    ...
                    <a href="#security-settings" class="settings-tab">
                        <span class="dashicons dashicons-shield"></span>
                        <?php echo esc_html__('Security', 'wallet-up-login-customizer'); ?>
                    </a>
                </div>
                
                <div class="settings-panels">
                    <form method="post" action="options.php" id="wallet-up-settings-form">
                        ...
                        <div id="advanced-settings" class="settings-panel">
                            ...
                            <p><?php echo esc_html__('Configure advanced options and manage your settings.', 'wallet-up-login-customizer'); ?></p>
                            ...
                            <h3><?php echo esc_html__('Settings Management', 'wallet-up-login-customizer'); ?></h3>
                            <p><?php echo esc_html__('Export your current settings, import from a backup, or reset to defaults.', 'wallet-up-login-customizer'); ?></p>
                            ...
                            <div class="settings-actions">
                                <div class="settings-action-item">
                                    ...
                                    <p class="description"><?php echo esc_html__('Download your current settings as a JSON file.', 'wallet-up-login-customizer'); ?></p>
                                </div>
                                ...
                                <div class="settings-action-item">
                                    ...
                                    <p class="description"><?php echo esc_html__('Upload a previously exported settings file.', 'wallet-up-login-customizer'); ?></p>
                                </div>
                                
                                <div class="settings-action-item">
                                    <button type="button" id="reset-settings" class="button button-secondary button-reset">
                                        <span class="dashicons dashicons-image-rotate"></span>
                                        <?php echo esc_html__('Reset to Defaults', 'wallet-up-login-customizer'); ?>
                                    </button>
                                    <p class="description" style="color: #d63638;"><?php echo esc_html__('Warning: This will reset all settings to their default values.', 'wallet-up-login-customizer'); ?></p>
                                </div>
                            </div>
                            ...
                            jQuery(document).ready(function($) {
                                ...
                                $('#export-settings').on('click', function(e) {
                                    e.preventDefault();
                                    window.location.href = '<?php echo admin_url('admin-ajax.php?action=wallet_up_export_settings&nonce=' . wp_create_nonce('wallet_up_export')); ?>';
                                });
                                ...
                                $('#import-file').on('change', function(e) {
                                    ...
                                    reader.onload = function(e) {
                                        if (confirm('<?php echo esc_js(__('Are you sure you want to import these settings? This will overwrite your current settings.', 'wallet-up-login-customizer')); ?>')) {
                                            var formData = new FormData();
                                            formData.append('action', 'wallet_up_import_settings');
                                            formData.append('nonce', '<?php echo wp_create_nonce('wallet_up_import'); ?>');
                                            formData.append('settings', e.target.result);
                                ...
                                $('#reset-settings').on('click', function(e) {
                                    e.preventDefault();
                                    if (confirm('<?php echo esc_js(__('Are you sure you want to reset all settings to defaults? This cannot be undone.', 'wallet-up-login-customizer')); ?>')) {
                                        $.post(ajaxurl, {
                                            action: 'wallet_up_reset_settings',
                                            nonce: '<?php echo wp_create_nonce('wallet_up_reset'); ?>'
                                        }, function(response) {
                                            if (response.success) {
                                                alert(response.data.message);
                                                location.reload();
                        ...
                        <div id="security-settings" class="settings-panel">
                            <h2><?php echo esc_html__('Enterprise Security', 'wallet-up-login-customizer'); ?></h2>
                            <p><?php echo esc_html__('Advanced security features for protecting your login system.', 'wallet-up-login-customizer'); ?></p>
                            
                            <?php
                            // SIMPLIFIED: Get security options from one source with proper defaults
                            $defaults = array(
                            ...
                            $stored = get_option('wallet_up_login_customizer_security_options', array());
                            $security_options = wp_parse_args($stored, $defaults);
                            ?>
                            ...
                            <table class="form-table">
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" id="force_login_enabled" name="wallet_up_login_customizer_security_options[force_login_enabled]" value="1" <?php checked(!empty($security_options['force_login_enabled'])); ?>>
                                        <label for="force_login_enabled"><?php esc_html_e('Require users to login before accessing the website', 'wallet-up-login-customizer'); ?></label>
                                        <p class="description"><?php esc_html_e('Visitors must authenticate before viewing any content. Registration and password reset still work normally.', 'wallet-up-login-customizer'); ?></p>
                                        <p class="description force-login-warning" style="color: #d63638; font-style: italic; margin-top: 10px; <?php echo empty($security_options['force_login_enabled']) ? 'display: none;' : ''; ?>"><?php echo esc_html__('⚠️ Important: Force Login may not work properly with aggressive page caching (LiteSpeed, Cloudflare, etc.). For best results, clear your cache when this feature is enabled. The plugin attempts to bypass caching, but server-level caching may still serve cached pages to non-logged-in users.', 'wallet-up-login-customizer'); ?></p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" id="hide_wp_login" name="wallet_up_login_customizer_security_options[hide_wp_login]" value="1" <?php checked(!empty($security_options['hide_wp_login'])); ?>>
                                        <label for="hide_wp_login"><?php esc_html_e('Hide and protect default WordPress login URL', 'wallet-up-login-customizer'); ?></label>
                                    </td>
                                </tr>
                                
                                <tr>
                                    <th scope="row"><label for="custom_login_slug"><?php esc_html_e('Custom Login URL', 'wallet-up-login-customizer'); ?></label></th>
                                    <td>
                                        ...
                                        <p class="description"><?php esc_html_e('Custom URL slug for the login page (e.g., secure-login)', 'wallet-up-login-customizer'); ?></p>
                                        <p class="description" style="margin-top: 10px;">
                                            <strong><?php esc_html_e('Your login URL will be:', 'wallet-up-login-customizer'); ?></strong><br>
                                            <code id="custom-login-url-preview" style="background: #f0f0f1; padding: 5px; display: inline-block; margin-top: 5px;">
                                                <?php echo esc_url(home_url('?' . $security_options['custom_login_slug'] . '=1')); ?>
                                            </code>
                                            <button type="button" id="copy-login-url" class="button button-small" style="margin-left: 10px;">
                                                <?php esc_html_e('Copy', 'wallet-up-login-customizer'); ?>
                                            </button>
                                            <span id="copy-success" style="color: green; margin-left: 10px; display: none;">
                                                <?php esc_html_e('Copied!', 'wallet-up-login-customizer'); ?>
                                            </span>
                                        </p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" id="max_login_attempts" name="wallet_up_login_customizer_security_options[max_login_attempts]" value="<?php echo esc_attr($security_options['max_login_attempts']); ?>" min="3" max="10">
                                        <p class="description"><?php esc_html_e('Maximum failed login attempts before temporary lockout.', 'wallet-up-login-customizer'); ?></p>
                                    </td>
                                </tr>
                                
                                <tr>
                                    <th scope="row"><label for="lockout_duration"><?php esc_html_e('Lockout Duration', 'wallet-up-login-customizer'); ?></label></th>
                                    <td>
                                        <select id="lockout_duration" name="wallet_up_login_customizer_security_options[lockout_duration]">
                                            <option value="300" <?php selected($security_options['lockout_duration'], 300); ?>><?php esc_html_e('5 minutes', 'wallet-up-login-customizer'); ?></option>
                                            <option value="600" <?php selected($security_options['lockout_duration'], 600); ?>><?php esc_html_e('10 minutes', 'wallet-up-login-customizer'); ?></option>
                                            <option value="900" <?php selected($security_options['lockout_duration'], 900); ?>><?php esc_html_e('15 minutes', 'wallet-up-login-customizer'); ?></option>
                                            <option value="1800" <?php selected($security_options['lockout_duration'], 1800); ?>><?php esc_html_e('30 minutes', 'wallet-up-login-customizer'); ?></option>
                                            <option value="3600" <?php selected($security_options['lockout_duration'], 3600); ?>><?php esc_html_e('60 minutes', 'wallet-up-login-customizer'); ?></option>
                                        </select>
                                        <p class="description"><?php esc_html_e('How long to lock out users after too many failed attempts.', 'wallet-up-login-customizer'); ?></p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" id="email_notifications_enabled" name="wallet_up_login_customizer_security_options[email_notifications_enabled]" value="1" <?php checked(!empty($security_options['email_notifications_enabled'])); ?>>
                                ...
                                <tr class="email-notification-settings" <?php echo empty($security_options['email_notifications_enabled']) ? 'style="display:none;"' : ''; ?>>
                                    <th scope="row"><label for="email_notification_threshold"><?php esc_html_e('Alert Threshold', 'wallet-up-login-customizer'); ?></label></th>
                                    <td>
                                        <input type="number" id="email_notification_threshold" name="wallet_up_login_customizer_security_options[email_notification_threshold]" value="<?php echo esc_attr($security_options['email_notification_threshold'] ?? 3); ?>" min="1" max="10">
                                ...
                                <tr class="email-notification-settings" <?php echo empty($security_options['email_notifications_enabled']) ? 'style="display:none;"' : ''; ?>>
                                    ...
                                    <td>
                                        <input type="email" id="email_recipient" name="wallet_up_login_customizer_security_options[email_recipient]" value="<?php echo esc_attr($security_options['email_recipient'] ?? get_option('admin_email')); ?>" class="regular-text">
                                        <button type="button" id="test-email-button" class="button"><?php esc_html_e('Send Test Email', 'wallet-up-login-customizer'); ?></button>
                                ...
                                <tr class="email-notification-settings" <?php echo empty($security_options['email_notifications_enabled']) ? 'style="display:none;"' : ''; ?>>
                                    ...
                                    <td>
                                        <input type="checkbox" id="email_digest_enabled" name="wallet_up_login_customizer_security_options[email_digest_enabled]" value="1" <?php checked(!empty($security_options['email_digest_enabled'])); ?>>
                                ...
                                <tr>
                                    <th scope="row"><label for="whitelist_ips"><?php esc_html_e('Whitelisted IPs', 'wallet-up-login-customizer'); ?></label></th>
                                    <td>
                                        <textarea id="whitelist_ips" name="wallet_up_login_customizer_security_options[whitelist_ips]" rows="5" class="large-text"><?php 
                                            $whitelist_ips = isset($security_options['whitelist_ips']) ? $security_options['whitelist_ips'] : [];
                                            if (is_array($whitelist_ips)) {
                                                echo esc_textarea(implode("\n", $whitelist_ips));
                                            } elseif (is_string($whitelist_ips)) {
                                                echo esc_textarea($whitelist_ips);
                                            }
                                        ?></textarea>
                                        <p class="description"><?php esc_html_e('Enter IP addresses to whitelist (one per line). These IPs will bypass rate limiting.', 'wallet-up-login-customizer'); ?></p>
                                        <p class="description"><?php 
                                            $current_ip = isset($_SERVER['REMOTE_ADDR']) ? sanitize_text_field($_SERVER['REMOTE_ADDR']) : 'Unknown';
                                            if (filter_var($current_ip, FILTER_VALIDATE_IP)) {
                                                echo sprintf(esc_html__('Your current IP: %s', 'wallet-up-login-customizer'), esc_html($current_ip));
                                            } else {
                                                echo esc_html__('Your current IP: Unknown', 'wallet-up-login-customizer');
                                            }
                                        ?></p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="checkbox" id="security_headers" name="wallet_up_login_customizer_security_options[security_headers]" value="1" <?php checked(!empty($security_options['security_headers'])); ?>>
                                        <label for="security_headers"><?php esc_html_e('Enable security headers (X-Frame-Options, CSP, etc.)', 'wallet-up-login-customizer'); ?></label>
                                        <p class="description security-headers-warning" style="color: #d63638; font-style: italic; margin-top: 10px; <?php echo empty($security_options['security_headers']) ? 'display: none;' : ''; ?>"><?php echo esc_html__('⚠️ Important: Security Headers (especially Content-Security-Policy) may conflict with other plugins or themes that use inline scripts/styles. If you experience broken functionality after enabling this, check your browser console for CSP violations and consider disabling this feature or adjusting your CSP policy.', 'wallet-up-login-customizer'); ?></p>
                                    </td>
                                </tr>
                            </table>
                        </div>
        ...
        <script type="text/javascript">
        jQuery(document).ready(function($) {
            // Force login warning toggle
            $('#force_login_enabled').change(function() {
                if ($(this).is(':checked')) {
                    $('.force-login-warning').fadeIn();
                } else {
                    $('.force-login-warning').fadeOut();
                }
            });
            
            // Security headers warning toggle
            $('#security_headers').change(function() {
                if ($(this).is(':checked')) {
                    $('.security-headers-warning').fadeIn();
                } else {
                    $('.security-headers-warning').fadeOut();
                }
            });
            
            // Email notification toggle
            $('#email_notifications_enabled').change(function() {
                if ($(this).is(':checked')) {
            ...
            // Dynamic custom login URL preview
            $('#custom_login_slug').on('input', function() {
                var slug = $(this).val() || 'secure-login';
                var homeUrl = '<?php echo esc_js(home_url('/')); ?>';
                var newUrl = homeUrl + '?' + slug + '=1';
                $('#custom-login-url-preview').text(newUrl);
            });
            
            // Copy login URL to clipboard
            $('#copy-login-url').click(function(e) {
                e.preventDefault();
                var urlText = $('#custom-login-url-preview').text();
            ...
            $('#test-email-button').click(function(e) {
                ...
                $.post(ajaxurl, {
                    ...
                    email: $('#email_recipient').val()
                }, function(response) {
                    $button.prop('disabled', false).text(originalText);
                    
                    if (response.success) {
                        // Handle both response.data.message and response.data as string
                        var message = (typeof response.data === 'object' && response.data.message) ? response.data.message : response.data;
                        $result.html('<span style="color: green;">✓ ' + (message || 'Test email sent successfully') + '</span>');
                    } else {
                        var error = (typeof response.data === 'object' && response.data.message) ? response.data.message : response.data;
...
public function ajax_export_settings() {
    ...
    if (!isset($_GET['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['nonce'])), 'wallet_up_export') || !current_user_can('manage_options')) {
        wp_die('Unauthorized');
    }
    
    // Get ALL settings - including all option groups
    $settings = array(
        'type' => 'wallet_up_login_customizer',
        'version' => WALLET_UP_LOGIN_CUSTOMIZER_VERSION,
        'exported' => current_time('mysql'),
        'site_url' => get_site_url(),
        'settings' => get_option('wallet_up_login_customizer_options', array()),
        'security_settings' => get_option('wallet_up_login_customizer_security_options', array()),
        'logo_settings' => get_option('wallet_up_logo_settings', array()),
        'files_installed' => get_option('wallet_up_files_installed', false),
        'files_last_sync' => get_option('wallet_up_files_last_sync', false),
        'sync_log' => get_option('wallet_up_sync_log', array())
    );
    
    // Set headers for download
    header('Content-Type: application/json');
    header('Content-Disposition: attachment; filename="wallet-up-settings-' . date('Y-m-d-His') . '.json"');
    header('Cache-Control: no-cache, no-store, must-revalidate');
...
public function ajax_import_settings() {
    // Check nonce and permissions
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'wallet_up_import') || !current_user_can('manage_options')) {
        wp_send_json_error(array('message' => __('Unauthorized', 'wallet-up-login-customizer')));
    }
    
    // Get and validate the settings data
    $json_data = isset($_POST['settings']) ? wp_unslash($_POST['settings']) : '';
    $data = json_decode($json_data, true);
    
    if (json_last_error() !== JSON_ERROR_NONE) {
        wp_send_json_error(array('message' => __('Invalid JSON data', 'wallet-up-login-customizer')));
    }
    
    // Verify it's a valid export
    if (!isset($data['type']) || $data['type'] !== 'wallet_up_login_customizer') {
        wp_send_json_error(array('message' => __('Invalid settings file', 'wallet-up-login-customizer')));
    }
    
    // Sanitize and import ALL settings
    if (isset($data['settings']) && is_array($data['settings'])) {
        $sanitized_settings = $this->sanitize_options($data['settings']);
        update_option('wallet_up_login_customizer_options', $sanitized_settings);
    }
    
    if (isset($data['security_settings']) && is_array($data['security_settings'])) {
        // Use Enterprise Security sanitization if available
        if (class_exists('WalletUpEnterpriseSecurity') && method_exists('WalletUpEnterpriseSecurity', 'sanitize_security_options')) {
            $sanitized_security = WalletUpEnterpriseSecurity::sanitize_security_options($data['security_settings']);
            update_option('wallet_up_login_customizer_security_options', $sanitized_security);
        }
    }
    
    // Import additional settings
    if (isset($data['logo_settings']) && is_array($data['logo_settings'])) {
        update_option('wallet_up_logo_settings', $data['logo_settings']);
    }
    
    if (isset($data['files_installed'])) {
        update_option('wallet_up_files_installed', $data['files_installed']);
    }
    
    if (isset($data['files_last_sync'])) {
        update_option('wallet_up_files_last_sync', $data['files_last_sync']);
    }
    
    if (isset($data['sync_log']) && is_array($data['sync_log'])) {
        update_option('wallet_up_sync_log', $data['sync_log']);
    }
    
    wp_send_json_success(array('message' => __('Settings imported successfully!', 'wallet-up-login-customizer')));
}
...
public function ajax_reset_settings() {
    // Check nonce and permissions
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'wallet_up_reset') || !current_user_can('manage_options')) {
        wp_send_json_error(array('message' => __('Unauthorized', 'wallet-up-login-customizer')));
    }
    ...
    // Reset security settings too
    delete_option('wallet_up_login_customizer_security_options');
    
    wp_send_json_success(array('message' => __('Settings reset to defaults successfully!', 'wallet-up-login-customizer')));
}

/**
 * Handle AJAX test security email request
 */
public function ajax_test_security_email() {
    // Check nonce and permissions
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'wallet_up_test_email') || !current_user_can('manage_options')) {
        wp_send_json_error(esc_html__('Unauthorized', 'wallet-up-login-customizer'));
    }
    ...
    if (!is_email($email)) {
        wp_send_json_error(esc_html__('Invalid email address', 'wallet-up-login-customizer'));
    }
    
    // Prepare the test email - decode HTML entities for plain text email
    $site_name = html_entity_decode(get_bloginfo('name'), ENT_QUOTES, 'UTF-8');
    $subject = sprintf('[%s] Security Alert Test', $site_name);
    $message = sprintf(
        "This is a test email from the Wallet Up Login Customizer security system.\n\n" .
        "If you received this email, your security notifications are working correctly.\n\n" .
        "Site: %s\n" .
        "Time: %s\n\n" .
        "This is a test notification only. No security event has occurred.",
        home_url(),
        current_time('mysql')
    );
    ...
    if ($sent) {
        wp_send_json_success(array('message' => sprintf('Test email sent to %s', $email)));
    } else {
        wp_send_json_error(esc_html__('Failed to send test email. Please check your email configuration.', 'wallet-up-login-customizer'));
    }
}
